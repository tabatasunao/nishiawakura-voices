...
export default function ProposalForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await proposeQuestion(title.trim(), body.trim(), category)
      if (result.error) {
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-1.5">
        ...
        <select
          ...
          className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#2D6A4F]"
        >
          {CATEGORIES.map(c => (
            <option key={c} value={c}>{ct(c as Parameters<typeof ct>[0])}</option>
          ))}
        </select>
      </div>
